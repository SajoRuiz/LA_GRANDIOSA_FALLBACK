...
export default async function OrderAssetsPage({ params }: { params: { orderId: string } }) {
  ...
  const { data: order } = await admin.from("orders").select("id,order_number,status,agency_id,asset_due_at,asset_due_note").eq("id", params.orderId).eq("agency_id", access.agency.id).maybeSingle();
  if (!order) notFound();
