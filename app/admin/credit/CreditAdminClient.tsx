...
export default function CreditAdminClient({
  ...
  async function resolveReview(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error ?? "Credit review update failed.");
      }

      setMessage(
        `${result.orderNumber ?? "Order"} credit exception ${
  ...
  async function addAdjustment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error ?? "Credit adjustment failed.");
      }
