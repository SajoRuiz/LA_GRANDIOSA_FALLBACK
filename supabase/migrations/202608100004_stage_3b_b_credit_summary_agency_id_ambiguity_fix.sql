-- Stage 3B-B hotfix follow-up
-- Fix remaining ambiguous references in get_agency_credit_summary.
...
begin
  ...
  select greatest(coalesce(sum(ledger.amount_cents), 0), 0)
  into v_ledger
  from public.agency_credit_ledger ledger
  where ledger.agency_id = p_agency_id;

  select coalesce(sum(hold.amount_cents), 0)
  into v_active_holds
  from public.agency_credit_holds hold
  where hold.agency_id = p_agency_id
    and hold.status in ('active', 'approved_exception')
    and (hold.held_until is null or hold.held_until > timezone('utc', now()));

  select coalesce(sum(hold.amount_cents), 0)
  into v_pending
  from public.agency_credit_holds hold
  where hold.agency_id = p_agency_id
    and hold.status = 'pending_exception'
    and (hold.held_until is null or hold.held_until > timezone('utc', now()));
